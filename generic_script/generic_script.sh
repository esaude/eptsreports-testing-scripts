...
#creating the folder "/epts_generic"  in home user  directory if  it  does not exist
echo "Creating the  'epts_generic' folder  in home user directory if  it  does not exist........"
if [ ! -d "~/epts_" ];
	then 
		mkdir ~/epts_generic
fi
...
#Files will be written in this directory then copied to ~/epts_generic
...
#Asking what client wants to work on
printf "What do you want to work on?\n1 - Metadata finds\n2 - Find databases that have GAAC tables\n3 - Want to source a file\n"
read option
...
if [ $option == "1" ]
        ...
        #moving now all the files to ~/epts_generic
        cp -v -r $mysqlPath* ~/epts_generic/
        ls ~/epts_generic/
elif [ $option == "2" ]
then
        declare -a  tabelas=()
        #write all the out put to a file for show the tables
        i=0
        for  db in "${db_list[@]}"
                do
    		mysql -u $mysql_user -p$mysql_passwd -h $host -D$db -e "$querie" >  file.txt
                ...
                if [[ $p == "2" ]]
                        then
                         ...
                         tabelas[$i]="$db has gaac\n"      
                else
                         tabelas[$i]="$db has no gaac\n"
                fi
                let i++
        done
        echo ""
        echo ""
        echo " Final list ${tabelas[@]}"
        cat <<< ${tabelas[@]} > database.txt
elif [ $option == "3" ]
then
        echo "What is the name of the file? (place it in a folder that you running this script)"
        read file
        for  db in "${db_list[@]}"
                do
                echo "   " 
                ...
                #      echo " Here is the query: $sql" ## for debugging
                mysql -u $mysql_user -p$mysql_passwd -h $host -D$db -e "source $file"
...
echo "The files are located in '~/epts_generic' in your home directory "
