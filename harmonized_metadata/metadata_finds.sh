...
queries[0,0]="SELECT program_id,concept_id,creator,date_created,changed_by,date_changed,retired,name,description,uuid,outcomes_concept_id FROM program ORDER BY program_id"
queries[0,1]="all-programs"

#--Get all the workflow of the programs
queries[1,0]="SELECT p.program_workflow_id,p.program_id,p.creator,p.date_created,p.changed_by,p.date_changed,p.retired program_workflow_retired, p.concept_id,c.name,p.uuid program_workflow_uuid,c.uuid concept_uuid FROM program_workflow p INNER JOIN concept_name c ON p.concept_id=c.concept_id ORDER BY program_workflow_id "
queries[1,1]="all-workflow"

#--Get all the workflow states
queries[2,0]="SELECT p.program_workflow_id,p.program_Workflow_state_id,p.initial,p.terminal,p.retired program_workflow_state_retired, p.concept_id, c.name, p.uuid program_workflow_state_uuid FROM program_workflow_state p INNER JOIN concept_name c ON p.concept_id=c.concept_id ORDER BY program_workflow_id"
queries[2,1]="all-workflow-states"

#--Get all the person attribute types
queries[3,0]="SELECT person_attribute_type_id,name,description,format,foreign_key,searchable,date_created,retired,retire_reason,date_retired,uuid FROM  person_attribute_type ORDER BY person_attribute_type_id"
queries[3,1]="all-person-attributes"

#--Get all the encounter type
queries[4,0]="SELECT encounter_type_id, name,description, creator, retired,retire_reason,date_retired,uuid,view_privilege,edit_privilege FROM encounter_type ORDER BY encounter_type_id"
queries[4,1]="all-encounter-types"

#--Get all the forms
queries[5,0]="SELECT form_id,name,version,build,published,description,encounter_type,template,creator,retired,retired_reason,date_retired,uuid FROM form"
queries[5,1]="all-forms"

#--Get all patient identifier type
queries[6,0]="SELECT patient_identifier_type_id,name,description,format,check_digit,creator,validator,required,format_description,retired,retire_reason,date_retired,uuid,location_behavior,uniqueness_behavior FROM  patient_identifier_type"
queries[6,1]="all-identifier-types"


#--Get all visit types
queries[7,0]="SELECT visit_type_id,name,description,creator,retired,retire_reason,date_retired,uuid FROM visit_type ORDER BY visit_type_id"
queries[7,1]="all-visit-types"

#--Get all concepts
queries[8,0]="SELECT c.concept_id,cn.name,cn.concept_name_type,c.retired,c.retired_by,c.date_retired,c.retire_reason,c.uuid FROM concept c INNER JOIN concept_name cn on cn.concept_id=c.concept_id ORDER BY c.concept_id"
queries[8,1]="all-concepts"
